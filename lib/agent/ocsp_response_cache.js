...
function OcspResponseCache()
{
  ...
  this.set = function set(cert, response)
  {
    ...
    {
      Logger.getInstance().debug("Failed to add certificate %s to OCSP cache file. err: %s", certId, e);
    }
  };
  ...
  this.get = function get(cert)
  {
    ...
    {
      Logger.getInstance().debug("Failed to get certificate %s. err: %s", certId, e);
    }
  };
